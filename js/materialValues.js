...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCellValue(anumb, clnum, newValue) {
        ...
        // Отправка AJAX запроса на сервер
        const xhr = new XMLHttpRequest();
        xhr.open('POST', '../function/update-clpr1.php', true);
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                // Обработка ответа от сервера, если необходимо
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCellValue(anumb, clnum, newValue) {
        ...
        // Отправка AJAX запроса на сервер
        const xhr = new XMLHttpRequest();
        xhr.open('POST', '../function/update-clpr2.php', true);
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                // Обработка ответа от сервера, если необходимо
...
function updateDatabase(anumb, cname, clprc, clpr1, clpr2) {
    ...
    xhr.open('POST', '../function/import_excel.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.onreadystatechange = function() {
